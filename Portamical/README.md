...
var args = testData.ToDistinctRowArray(ArgsCode.Instance);
// Result: [[testData1], [testData2]]

// Or flatten properties
var flatArgs = testData.ToDistinctRowArray(ArgsCode.Properties);
// Result: [[2, 3, 5], [5, 7, 12]]
```
...
// Identity conversion
var distinct = testData.ToDistinctRowArray(td => td);
// Result: 2 elements

// Convert to argument arrays
var args = testData.ToDistinctRowArray(td => td.ToArgs(ArgsCode.Instance));
// Result: [[testData1], [testData2]]

// Custom row conversion
var rows = testData.ToDistinctRowArray(td => new 
{ 
...
```csharp
// Simple deduplication (identity conversion)
TTestData[] ToDistinctRowArray<TTestData>(
    this IEnumerable<TTestData> testDataCollection)

// Convert to argument arrays
object?[][] ToDistinctRowArray<TTestData>(
    this IEnumerable<TTestData> testDataCollection,
    ArgsCode argsCode)
...
TRow[] ToDistinctRowArray<TTestData, TRow>(
    this IEnumerable<TTestData> testDataCollection,
...
2. **Collection Converters**
   - `CollectionConverter` (root namespace) - Synchronous deduplication and array conversion
     - `ToDistinctRowArray<TTestData, TRow>()` - Core deduplication with custom conversion
