...
// Performance:
// - < 10 items: Uses Task.FromResult (avoids Task.Run overhead ~5-20 탎)
...
| Small collection (&lt;10) | ~1-3 탎 | O(n) | 0 bytes (Task.FromResult) |
| Large collection (?10) | ~6-22 탎 | O(n) | Task.Run overhead |
...
- **Synchronous**: ~1-2 탎
- **Task-based (sync path)**: ~1-3 탎 (Task.FromResult)
- **Task-based (async path)**: ~6-22 탎 (Task.Run overhead)
...
- **Synchronous**: ~1-2 탎 for small collections (&lt;10 items)
- **Task-based (sync path)**: ~1-3 탎 with `Task.FromResult`
- **Task-based (async path)**: ~6-22 탎 with `Task.Run` overhead
