...
/// <summary>
/// Provides xUnit v2-specific assertion methods that extend Portamical's framework-agnostic
...
/// </para>
/// <para>
/// This class adapts xUnit v2's assertion API to Portamical's framework-agnostic assertion
...
/// </para>
/// <para>
/// <strong>⚠️ xUnit v2 Limitation - No Multiple Assertions:</strong>
/// </para>
/// <para>
/// Unlike NUnit 4.x's <c>Assert.Multiple</c>, xUnit v2 does <strong>not</strong> have a multiple assertion
/// feature. The <see cref="AssertMultiple"/> and <see cref="AssertMultipleAsync"/> methods are provided
/// for API consistency with NUnit/MSTest but are implemented as no-ops (they simply execute the assertions
/// directly without collecting failures):
/// <list type="bullet">
///   <item><description>
///     <strong>NUnit 4.x:</strong> <c>AssertMultiple</c> collects all assertion failures and reports them together
///   </description></item>
///   <item><description>
///     <strong>xUnit v2:</strong> Test stops at the first assertion failure (standard xUnit behavior)
///   </description></item>
/// </list>
/// </para>
/// <para>
/// <strong>xUnit v2 Assertion Style:</strong>
/// </para>
/// <para>
/// xUnit v2 uses a different assertion style compared to NUnit:
/// <list type="bullet">
///   <item><description>
///     <strong>xUnit v2:</strong> Static methods like <c>Assert.Equal(expected, actual)</c>
///   </description></item>
///   <item><description>
...
///   ↓ inherits
/// Portamical.xUnit.Assertions.PortamicalAssert (xUnit v2-specific adapter) ← This class
///   ↓ used by
...
/// </remarks>
/// <example>
/// <para><strong>Example 1: Multiple Assertions (xUnit v2 Limitation)</strong></para>
/// <code>
/// using Xunit;
...
///         
///         // ⚠️ xUnit v2 limitation: Stops at first failure
///         AssertMultiple(() =>
///         {
...
///         Assert.NotNull(preferences);  // ← Stops here if fails (xUnit v2 limitation)
///         
...
public abstract class PortamicalAssert : Portamical.Assertions.PortamicalAssert
{
    ...
    /// <remarks>
    /// <para>
    /// <strong>⚠️ xUnit v2 Limitation - No Multiple Assertion Collection:</strong>
    /// </para>
    /// <para>
    /// Unlike NUnit 4.x's <c>Assert.Multiple</c>, xUnit v2 does <strong>not</strong> have a feature to collect
    /// multiple assertion failures. This method is implemented as a no-op (it simply executes the delegate directly)
    /// and is provided only for API consistency with NUnit and MSTest adapters.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// <code>
    /// // xUnit v2 (this method - stops at first failure):
    /// AssertMultiple(() =>
    /// {
    ...
    /// <summary>
    /// Executes multiple asynchronous assertions. ⚠️ <strong>xUnit v2 Limitation:</strong> Stops at first failure (no-op wrapper).
    /// </summary>
    /// <param name="assertions">
    ...
    /// it is implemented as a no-op (it simply returns the <see cref="Task"/> directly) because xUnit v2 does not
    ...
    /// <list type="bullet">
    ///   <item><description>xUnit v2 has no multiple assertion scope to manage</description></item>
    ...
    ///         Assert.NotNull(profile);  // ← Stops here if fails (xUnit v2 limitation)
    ///         
    ...
    /// </para>
    /// <para>
    /// xUnit v2 uses static methods for assertions, unlike NUnit's constraint model:
    /// </para>
    /// <code>
    /// // xUnit v2 (used by this method):
    ...
    /// <remarks>
    /// <para>
    /// This method performs comprehensive exception validation using xUnit v2's exception recording and assertion APIs:
    /// <list type="number">
    ///   <item><description>
    ...
    /// </para>
    /// <para>
    /// <strong>xUnit v2's Record.Exception:</strong>
    /// </para>
    /// <para>
    /// xUnit v2 provides <c>Record.Exception</c> for capturing exceptions without rethrowing them,
    ...
    /// failures because xUnit v2 lacks this feature. If both type and message are wrong, only the first failure
    /// is reported.
    /// </para>
    /// </remarks>
    /// <example>
    ...
    /// </code>
    /// 
    /// <para><strong>Example 3: xUnit v2 Limitation (Stops at First Failure)</strong></para>
    /// <code>
    /// [Fact]
    ...
    ///     }, expected);
    ///     
    ///     // Test Output (xUnit v2 - stops at first failure):
