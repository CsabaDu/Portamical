...
public class TestDataFactoryTests
{
    ...
    {
        var actual = new ArgumentException("bad value");
        var sut = TestDataFactory.CreateTestDataThrows<ArgumentException, int>(Def, actual, 1);
        Assert.IsInstanceOfType<TestDataThrows<ArgumentException, int>>(sut);
        Assert.AreSame(sut.Expected, actual);
    ...
    public void CreateTestDataThrows_withArgumentNullException_usesConcreteTypeName()
    {
        ...
        var sut = TestDataFactory.CreateTestDataThrows<ArgumentNullException, int>(Def, actual, 1);
